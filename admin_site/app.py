...
def login():
	...
	cursor = conn.cursor()
	result = cursor.execute("SELECT * from login where binary username=%s and binary password=%s",[user,paswd])
	if(result is 1):
...
def signup():
	...
	facultyid = str(request.form["fact_id"])
	dept = str(request.form["fdept_id"])
	cursor = conn.cursor()
	result = cursor.execute("SELECT * from login where binary username=%s",[user])
	print (result)
	if(result == 1):
		return render_template("signup.html",title="SignUp",uname=user,msg="already present")
	cursor.execute("INSERT INTO faculty (faculty_id,fname,email,fdept) VALUES(%s, %s, %s, %s)",[facultyid,user,email,dept])
	cursor.execute("INSERT INTO login (faculty_id,username,password) VALUES(%s, %s, %s)",(facultyid,user,paswd))
...
def signup_student():
	...
	branch = str(request.form["stu_branch"])
	phone = str(request.form["stu_phone"])
	cursor = conn.cursor()
	result = cursor.execute("SELECT * from student where binary roll_id=%s",[roll_id])
	print (result)
	if(result == 1):
		return render_template("upload.html",uname=user,msg=" already present")
	cursor.execute("INSERT INTO student (username,student_email,roll_id,branch,phone) VALUES(%s, %s, %s, %s, %s)",(user,email,roll_id,branch,phone))
	conn.commit()
	return render_template("upload.html",uname=user,msg=" successfully signup")



@app.route('/courses_available',methods=['POST'])
def courses_available():
	return render_template("course.html", title="Enter Courses Information")
@app.after_request
def set_response_headers(response):
    response.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate'
    response.headers['Pragma'] = 'no-cache'
...
@app.route('/courses_info',methods=['POST'])
def courses_info():
	courseid = str(request.form["cid"])
	cname = str(request.form["course"])
	credits = str(request.form["credits"])
	sem = str(request.form["sem"])
	cursor = conn.cursor()
	result = cursor.execute("INSERT INTO course VALUES(%s, %s, %s, %s)", [courseid, cname, credits, sem])
	conn.commit()
	if(result):
		return render_template("course.html", msg="course successfully added", title="Enter Courses Information")
